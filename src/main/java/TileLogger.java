...
public class TileLogger {
    ...
    public static void showInfo(Player player) {
        ...
        str += String.format("\n    Native:");
        str += String.format("\n        Tiles: %.3f | %.3f", memoryUsage(0) * 1e-6, memoryUsage(1) * 1e-6);
        str += String.format("\n        Players: %.3f | %.3f", memoryUsage(2) * 1e-6, memoryUsage(3) * 1e-6);
        str += String.format("\n        Configs: %.3f | %.3f", memoryUsage(4) * 1e-6, memoryUsage(5) * 1e-6);
        player.sendMessage(str);
    }
